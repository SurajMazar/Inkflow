...
describe('exception rendering', () => {
  it('passes application errors through', () => {
    expect(renderException(Errors.notFound('Board'))).toMatchObject({
      status: 404,
      code: 'NOT_FOUND',
  ...
  it('maps Prisma, Zod, throttler and HTTP exceptions', () => {
    expect(renderException(prismaError('P2002'))).toMatchObject({
      status: 409,
      code: 'CONFLICT',
    ...
    expect(renderException(zodError)).toMatchObject({
      status: 400,
      code: 'VALIDATION_FAILED',
      details: [{ path: ['a'] }],
    });
    expect(renderException(new ThrottlerException())).toMatchObject({
      status: 429,
      code: 'RATE_LIMITED',
    });
    expect(renderException(new NotFoundException('Cannot GET /x'))).toMatchObject({
      status: 404,
      code: 'NOT_FOUND',
      message: 'Cannot GET /x',
    });
    expect(renderException(new PayloadTooLargeException('File too large'))).toMatchObject({
      status: 413,
      code: 'PAYLOAD_TOO_LARGE',
    });
    expect(renderException(new BadRequestException('nope'))).toMatchObject({
      status: 400,
      code: 'VALIDATION_FAILED',
    });
    expect(renderException({ status: 413, type: 'entity.too.large' })).toMatchObject({
      status: 413,
      code: 'PAYLOAD_TOO_LARGE',
    });
  });
