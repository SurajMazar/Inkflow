...
export function EditorContextMenu() {
  ...
  const nonLinear = selected.filter((e) => e.type !== 'arrow' && e.type !== 'line' && e.type !== 'connector');

  return (
    <DropdownMenu open onOpenChange={close} modal={false}>
      <DropdownMenuTrigger asChild>
        <span aria-hidden="true" className="pointer-events-none fixed size-px" style={{ left: menu.x, top: menu.y }} />
      </DropdownMenuTrigger>
      <DropdownMenuContent align="start" className="w-56" data-inkflow-ui data-testid="context-menu" onCloseAutoFocus={(e) => e.preventDefault()}>
        {menu.target === 'locked' && (
          <>
            <Item editor={editor} action="arrange.unlock" label="Unlock" payload={{ ids: menu.elementId ? [menu.elementId] : [] }} />
        ...
        {menu.target === 'selection' && selectedCount > 0 ? (
          <>
            ...
            {selectedCount === 1 && <Item editor={editor} action="nav.editSelected" label="Edit text" shortcut="↩" />}
            <DropdownMenuSeparator />
            ...
            <DropdownMenuSub>
              ...
              <DropdownMenuSubContent data-inkflow-ui>
                <Item editor={editor} action="arrange.bringToFront" label="Bring to front" shortcut="⇧⌘]" />
                <Item editor={editor} action="arrange.bringForward" label="Bring forward" shortcut="⌘]" />
                <Item editor={editor} action="arrange.sendBackward" label="Send backward" shortcut="⌘[" />
                <Item editor={editor} action="arrange.sendToBack" label="Send to back" shortcut="⇧⌘[" />
                <DropdownMenuSeparator />
                <Item editor={editor} action="arrange.flipHorizontal" label="Flip horizontal" shortcut="⇧H" />
                <Item editor={editor} action="arrange.flipVertical" label="Flip vertical" shortcut="⇧V" />
              </DropdownMenuSubContent>
            </DropdownMenuSub>
            {selectedCount > 1 && (
              <DropdownMenuSub>
                ...
                <DropdownMenuSubContent data-inkflow-ui>
                  <Item editor={editor} action="arrange.alignLeft" label="Align left" shortcut="⌥A" />
                  <Item editor={editor} action="arrange.alignCenter" label="Align center" shortcut="⌥H" />
                  <Item editor={editor} action="arrange.alignRight" label="Align right" shortcut="⌥D" />
                  <Item editor={editor} action="arrange.alignTop" label="Align top" shortcut="⌥W" />
                  <Item editor={editor} action="arrange.alignMiddle" label="Align middle" shortcut="⌥V" />
                  <Item editor={editor} action="arrange.alignBottom" label="Align bottom" shortcut="⌥S" />
                  <DropdownMenuSeparator />
                  <Item editor={editor} action="arrange.distributeHorizontal" label="Distribute horizontally" />
                  <Item editor={editor} action="arrange.distributeVertical" label="Distribute vertically" />
                </DropdownMenuSubContent>
              </DropdownMenuSub>
            )}
            ...
            {canEdit && !hasFrameSelected && (
              <DropdownMenuSub>
                ...
                <DropdownMenuSubContent data-inkflow-ui>
                  {frames.map((f) => (
                    <Item key={f.id} editor={editor} action="frame.addSelection" label={`Add to “${f.name}”`} payload={{ frameId: f.id }} />
                  ))}
            ...
            {nonLinear.length > 1 && <Item editor={editor} action="diagram.autoLayout" label="Auto layout…" shortcut="⌥⇧L" />}
            <Item editor={editor} action="diagram.selectConnected" label="Select connected" shortcut="⌥⌘A" />
            {selectedCount === 1 && nonLinear.length === 1 && <Item editor={editor} action="diagram.addConnected" label="Add connected node" shortcut="⌘↩" />}
            {selectedCount === 1 && <Item editor={editor} action="element.link" label="Link…" />}
            <DropdownMenuSeparator />
            {canComment && (
              <DropdownMenuItem onSelect={() => ui.startComment({ world: menu.world, elementId: menu.elementId })}>Comment</DropdownMenuItem>
            )}
            {hasFrameSelected && <Item editor={editor} action="export.frame" label="Export frame…" />}
          ...
          <>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuCheckboxItem checked={grid} onCheckedChange={() => editor.actions.run('view.toggleGrid')}>
              Show grid
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem checked={snap} onCheckedChange={() => editor.actions.run('view.toggleSnap')}>
              Snap to objects
            </DropdownMenuCheckboxItem>
            ...
            <DropdownMenuSeparator />
            {canComment && (
              <DropdownMenuItem onSelect={() => ui.startComment({ world: menu.world, elementId: null })}>Add comment here</DropdownMenuItem>
            )}
