...
describe('comboFromKeyboardEvent', () => {
  it('maps ⌘ to Mod on macOS and orders modifiers like the engine', () => {
    expect(comboFromKeyboardEvent(key({ metaKey: true, shiftKey: true }), 'mac')).toBe(
      'Shift+Mod+K',
    );
    expect(
      comboFromKeyboardEvent(key({ metaKey: true, altKey: true, key: 'å', code: 'KeyA' }), 'mac'),
    ).toBe('Alt+Mod+A');
    expect(comboFromKeyboardEvent(key({ ctrlKey: true, metaKey: true }), 'mac')).toBe('Ctrl+Mod+K');
  });

  it('maps Ctrl to Mod elsewhere', () => {
    expect(comboFromKeyboardEvent(key({ ctrlKey: true, shiftKey: true }), 'other')).toBe(
      'Shift+Mod+K',
    );
    expect(
      comboFromKeyboardEvent(key({ ctrlKey: true, key: '[', code: 'BracketLeft' }), 'other'),
    ).toBe('Mod+[');
  });

  it('uses physical keys for letters, digits and punctuation', () => {
    expect(comboFromKeyboardEvent(key({ key: '!', code: 'Digit1', shiftKey: true }), 'other')).toBe(
      'Shift+1',
    );
    expect(comboFromKeyboardEvent(key({ key: '?', code: 'Slash', shiftKey: true }), 'other')).toBe(
      'Shift+/',
    );
    expect(comboFromKeyboardEvent(key({ key: 'ArrowLeft', code: 'ArrowLeft' }), 'other')).toBe(
      'ArrowLeft',
    );
  ...
  it('ignores modifier-only presses', () => {
    expect(
      comboFromKeyboardEvent(key({ key: 'Shift', code: 'ShiftLeft', shiftKey: true }), 'mac'),
    ).toBeNull();
    expect(
      comboFromKeyboardEvent(key({ key: 'Meta', code: 'MetaLeft', metaKey: true }), 'mac'),
    ).toBeNull();
  });
...
describe('conflict detection', () => {
  ...
  it('respects overrides (moved shortcuts no longer conflict, new ones do)', () => {
    ...
    expect(
      findConflicts('Alt+Shift+C', 'tool.rectangle', overrides, DEFAULT_SHORTCUTS, 'other').map(
        (c) => c.id,
      ),
    ).toEqual(['edit.copy']);
  });
  ...
  it('matches shifted symbols against their base key', () => {
    expect(
      findConflicts('Shift+/', 'tool.hand', {}, DEFAULT_SHORTCUTS, 'other').map((c) => c.id),
...
describe('overrides', () => {
  ...
  it('drops overrides equal to a single default and supports reset', () => {
    ...
    expect(resetOverride({ 'tool.hand': 'J', 'tool.rectangle': 'G' }, 'tool.hand')).toEqual({
      'tool.rectangle': 'G',
    });
  });
});
