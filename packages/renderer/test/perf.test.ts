  ...
  for (let i = 0; i < count; i++) {
    ...
    switch (i % 8) {
      ...
      case 4:
        ...
        break;
      case 5: {
        const pts: PressurePoint[] = Array.from({ length: 60 }, (_, k) => [
          k * 2,
          Math.sin(k / 5) * 15 + 15,
          0.5,
        ]);
...
describe('performance', () => {
  it('generates, culls and re-renders 10,000 mixed elements quickly', () => {
    ...
    const second = renderer.render(elements, {
      ...options,
      viewport: { ...options.viewport, x: 2010 },
    });
    ...
    const overview = renderer.render(elements, {
      ...options,
      viewport: { x: 0, y: 0, zoom: 0.25, width: 1920, height: 1080 },
      lowFidelity: true,
    });
    ...
    const all = {
      ...options,
      viewport: { x: -100, y: -100, zoom: 0.05, width: 1920, height: 1080 },
    };
