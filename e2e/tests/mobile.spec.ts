...
import { createBoard, registerAndVerify, sceneElements, uniqueUser } from '../helpers';
...
test('touch drawing, pinch zoom and the mobile toolbar', async ({ page }) => {
  ...
  const zoomBefore = await page.evaluate(
    () =>
      (window as unknown as { __inkflow: { editor: { state: { viewport: { zoom: number } } } } })
        .__inkflow.editor.state.viewport.zoom,
  );
