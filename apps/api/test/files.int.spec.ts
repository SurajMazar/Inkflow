...
import { createBoard, createOp, element, op, PNG_1X1, signUp, startApp, TestClient, type TestApp } from './helpers';
...
describe('files', () => {
  it('uploads, validates and serves images with safe headers', async () => {
    ...
    const png = await owner.upload<FileDto>('/api/files', { buffer: PNG_1X1, filename: 'dot.png', contentType: 'image/png' }, { boardId: board.id });
    expect(png.status).toBe(201);
    expect(png.body).toMatchObject({ boardId: board.id, mimeType: 'image/png', width: 1, height: 1, size: PNG_1X1.length, originalName: 'dot.png' });
    ...
    expect(content.headers['content-security-policy']).toBe("default-src 'none'; img-src data:; style-src 'unsafe-inline'; sandbox");
    ...
    const svg = await owner.upload<FileDto>('/api/files', { buffer: SAFE_SVG, filename: 'box.svg', contentType: 'image/svg+xml' }, { boardId: board.id });
    ...
    const fake = await owner.upload('/api/files', { buffer: Buffer.from('definitely not an image'), filename: 'x.png', contentType: 'image/png' }, { boardId: board.id });
    expect(fake.status).toBe(415);
    expect((fake.body as { error: { code: string } }).error.code).toBe('UNSUPPORTED_MEDIA_TYPE');
    // A PNG declared as JPEG.
    const mismatch = await owner.upload('/api/files', { buffer: PNG_1X1, filename: 'x.jpg', contentType: 'image/jpeg' }, { boardId: board.id });
    ...
    for (const evil of [
      ...
      const res = await owner.upload('/api/files', { buffer: Buffer.from(evil), filename: 'evil.svg', contentType: 'image/svg+xml' }, { boardId: board.id });
    ...
    const denied = await stranger.upload('/api/files', { buffer: PNG_1X1, filename: 'dot.png', contentType: 'image/png' }, { boardId: board.id });
    ...
    const noBoard = await owner.upload('/api/files', { buffer: PNG_1X1, filename: 'dot.png', contentType: 'image/png' });
  ...
  it('supports client-generated ids and maintains file references', async () => {
    ...
    const image = element('image', { id: 'img-1', fileId, status: 'pending', naturalWidth: 1, naturalHeight: 1 });
    ...
    const up = await owner.upload<FileDto>('/api/files', { buffer: PNG_1X1, filename: 'a.png', contentType: 'image/png' }, { boardId: board.id, fileId });
    expect(up.status).toBe(201);
    expect(up.body.id).toBe(fileId);
    // Upload completion links the pending element.
    expect(await t.prisma.fileReference.findMany({ where: { boardId: board.id }, select: { elementId: true, fileId: true } })).toEqual([
      { elementId: 'img-1', fileId },
    ]);
    ...
    expect(detail.body.document.files[fileId]).toMatchObject({ id: fileId, mimeType: 'image/png', url: `/api/files/${fileId}/content` });

    // Idempotent retry and conflicts.
    const retry = await owner.upload<FileDto>('/api/files', { buffer: PNG_1X1, filename: 'a.png', contentType: 'image/png' }, { boardId: board.id, fileId });
    expect(retry.status).toBe(201);
    expect(retry.body.id).toBe(fileId);
    const otherContent = await owner.upload('/api/files', { buffer: Buffer.from(SAFE_SVG), filename: 'b.svg', contentType: 'image/svg+xml' }, { boardId: board.id, fileId });
    expect(otherContent.status).toBe(409);
    const otherBoardSameId = await owner.upload('/api/files', { buffer: PNG_1X1, filename: 'a.png', contentType: 'image/png' }, { boardId: otherBoard.id, fileId });
    expect(otherBoardSameId.status).toBe(409);
    const badId = await owner.upload('/api/files', { buffer: PNG_1X1, filename: 'a.png', contentType: 'image/png' }, { boardId: board.id, fileId: 'nope' });
    expect(badId.status).toBe(400);

    // An op pointing at a file of another board is rejected.
    const foreign = element('image', { id: 'img-2', fileId, status: 'saved', naturalWidth: 1, naturalHeight: 1 });
    const cross = await owner.post<{ results: OpResult[] }>(`/api/boards/${otherBoard.id}/operations`, {
      clientId,
      batchId: 'b2',
    ...
    await owner.post(`/api/boards/${board.id}/operations`, {
      clientId,
      batchId: 'b4',
      ops: [op(clientId, { type: 'UPDATE_ELEMENT', elementId: 'img-1', patch: { isDeleted: false } })],
    });
  ...
  it('stores and serves board thumbnails', async () => {
    ...
    const put = await owner.upload(`/api/boards/${board.id}/thumbnail`, { buffer: PNG_1X1, filename: 't.png', contentType: 'image/png' }, {}, { method: 'put' });
    expect(put.status).toBe(200);
    const svg = await owner.upload(`/api/boards/${board.id}/thumbnail`, { buffer: SAFE_SVG, filename: 't.svg', contentType: 'image/svg+xml' }, {}, { method: 'put' });
    expect(svg.status).toBe(415);
    const summary = await owner.get<BoardDetailDto>(`/api/boards/${board.id}`);
    expect(summary.body.board.thumbnailUrl).toMatch(new RegExp(`^/api/boards/${board.id}/thumbnail\\?v=`));
