...
describe('boards', () => {
  it('creates, lists, renames, favorites, trashes, restores and deletes boards', async () => {
    ...
    const created = await owner.post<BoardSummaryDto>('/api/boards', {
      workspaceId: ws.id,
      title: 'Architecture',
    });
    ...
    const all = await owner.get<BoardSummaryDto[]>('/api/boards', {
    ...
    const recent = await owner.get<BoardSummaryDto[]>('/api/boards', {
      query: { filter: 'recent' },
    });
    ...
    const favorites = await owner.get<BoardSummaryDto[]>('/api/boards', {
    ...
    expect(
      (await owner.get<BoardSummaryDto[]>('/api/boards', { query: { filter: 'favorites' } })).body,
    ).toHaveLength(0);
    ...
    const emptied = await owner.post<{ deleted: number }>('/api/boards/trash/empty', {
      workspaceId: ws.id,
    });
  ...
  it('searches titles and element text of accessible boards only', async () => {
    ...
    expect(
      (await stranger.get<SearchResultDto[]>('/api/search', { query: { q: 'kangaroo' } })).body,
    ).toHaveLength(0);
  ...
  it('reports health and readiness', async () => {
    ...
    expect(
      Object.values(ready.body.checks).every(
        (c) => c.status === 'ok' && typeof c.latencyMs === 'number',
      ),
    ).toBe(true);
    const docs = await anon.get<{ openapi: string; paths: Record<string, unknown> }>(
      '/api/docs-json',
    );
