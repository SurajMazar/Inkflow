...
export class IconRegistry {
  ...
  register(def: IconDefinition): void {
    if (!/^[a-z0-9][a-z0-9-]{0,63}$/.test(def.key))
      throw new Error(`Invalid icon key "${def.key}"`);
    const valid = (d: string) =>
      d.length > 0 && d.length <= 10_000 && /^[MmLlHhVvCcSsQqTtAaZz0-9eE.,\s+-]+$/.test(d);
    if (def.paths.length === 0 && !(def.fills && def.fills.length > 0))
      throw new Error(`Icon "${def.key}" has no paths`);
    if (!def.paths.every(valid) || !(def.fills ?? []).every(valid))
      throw new Error(`Icon "${def.key}" has invalid path data`);
