...
const none = { isOwner: false, memberRole: null, workspaceRole: null, workspaceAccess: 'EDITOR', linkRole: null } as const;
...
describe('effective board role', () => {
  ...
  it('makes the owner and workspace admins OWNER', () => {
    ...
    expect(computeEffectiveRole({ ...none, workspaceRole: 'ADMIN', workspaceAccess: 'NONE' }).role).toBe('OWNER');
    expect(computeEffectiveRole({ ...none, workspaceRole: 'OWNER', workspaceAccess: 'NONE' }).role).toBe('OWNER');
  });
  ...
  it('takes the maximum of member role, workspace role and share link', () => {
    expect(computeEffectiveRole({ ...none, memberRole: 'VIEWER', workspaceRole: 'MEMBER', workspaceAccess: 'EDITOR' }).role).toBe('EDITOR');
    expect(computeEffectiveRole({ ...none, memberRole: 'EDITOR', workspaceRole: 'MEMBER', workspaceAccess: 'VIEWER' }).role).toBe('EDITOR');
    ...
    const linkNotNeeded = computeEffectiveRole({ ...none, memberRole: 'EDITOR', linkRole: 'VIEWER' });
    expect(linkNotNeeded).toEqual({ role: 'EDITOR', ownRole: 'EDITOR', viaShareLink: false });
    expect(computeEffectiveRole({ ...none, linkRole: 'VIEWER' })).toEqual({ role: 'VIEWER', ownRole: null, viaShareLink: true });
  });
});
