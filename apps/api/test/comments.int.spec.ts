...
describe('comments', () => {
  it('supports threads, mentions, notifications and resolution', async () => {
    ...
    const found = await alice.get<PublicUserDto[]>('/api/users/search', {
      query: { q: 'Bob', boardId: board.id },
    });
    expect(found.body.map((u) => u.id)).toEqual([bob.user!.id]);
    const notFound = await alice.get<PublicUserDto[]>('/api/users/search', {
      query: { q: 'Outsider', boardId: board.id },
    });
    ...
    expect(mention).toMatchObject({
      ...
      link: `/b/${board.id}?comment=${created.body.id}`,
    });
    ...
    expect((await outsider.get<NotificationListDto>('/api/notifications')).body.items).toHaveLength(
      0,
    );

    // Replies notify thread participants.
    const reply = await bob.post<CommentReplyDto>(`/api/comments/${created.body.id}/replies`, {
      body: 'Thanks!',
    });
    ...
    // Only authors edit.
    expect(
      (await bob.patch(`/api/comments/${created.body.id}`, { body: 'edited by bob' })).status,
    ).toBe(403);
    const edited = await alice.patch<CommentDto>(`/api/comments/${created.body.id}`, {
      body: 'Edited comment',
    });
    ...
    expect(
      (await alice.patch(`/api/comment-replies/${reply.body.id}`, { body: 'nope' })).status,
    ).toBe(403);
    expect(
      (
        await bob.patch<CommentReplyDto>(`/api/comment-replies/${reply.body.id}`, {
          body: 'Thanks a lot!',
        })
      ).body.body,
    ).toBe('Thanks a lot!');
    ...
    expect(
      (await alice.get<NotificationListDto>('/api/notifications')).body.items.some(
        (n) => n.type === 'COMMENT_RESOLVED',
      ),
    ).toBe(true);
    expect((await alice.get<CommentDto[]>(`/api/boards/${board.id}/comments`)).body).toHaveLength(
      0,
    );
    expect(
      (
        await alice.get<CommentDto[]>(`/api/boards/${board.id}/comments`, {
    ...
    expect(
      (await anon.get<CommentDto[]>(`/api/boards/${board.id}/comments`, { headers })).body,
    ).toHaveLength(1);
    expect(
      (await anon.post(`/api/comments/${created.body.id}/replies`, { body: 'anon' }, { headers }))
        .status,
    ).toBe(401);
    // Strangers see nothing.
    expect((await outsider.get(`/api/boards/${board.id}/comments`)).status).toBe(404);
    expect(
      (await outsider.post(`/api/comments/${created.body.id}/replies`, { body: 'x' })).status,
    ).toBe(404);
    ...
    expect((await owner.get<CommentDto[]>(`/api/boards/${board.id}/comments`)).body).toHaveLength(
      0,
    );
  });
});
