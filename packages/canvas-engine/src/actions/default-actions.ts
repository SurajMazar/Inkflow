...
export function registerDefaultActions(registry: ActionRegistry): void {
  ...
  r.register({
    id: 'edit.undo',
    label: 'Undo',
    enabled: (e) => e.state.canUndo,
    perform: (e) => e.undo(),
  });
  r.register({
    id: 'edit.redo',
    label: 'Redo',
    enabled: (e) => e.state.canRedo,
    perform: (e) => e.redo(),
  });
  r.register({
    id: 'edit.copy',
  ...
  r.register({
    ...
    perform: (e) => {
      ...
      for (const el of selected)
        if (el.type === 'frame')
          for (const c of e.scene.getFrameChildren(el.id)) withChildren.set(c.id, c);
      const offset = 16 / Math.max(0.1, e.state.viewport.zoom);
      const frameIds = new Set(e.getFrames().map((f) => f.id));
      const { elements, idMap } = duplicateElements([...withChildren.values()], {
        dx: offset,
        dy: offset,
  ...
  r.register({
    ...
    enabled: hasSelection,
    readOnlySafe: true,
    perform: (e) => e.clipboard.copyStyles(),
  });
  r.register({
    id: 'edit.pasteStyles',
    label: 'Paste styles',
  ...
  r.register({
    ...
    readOnlySafe: true,
    perform: (e) => e.requestUi({ type: 'find' }),
  });
  ...
  r.register({
    ...
    perform: (e) => {
      ...
      e.updateElements(
        patches.map(([id, p]) => [id, { groupIds: [...p.groupIds] }] as const),
        'Group',
      );
  ...
  const zorder = (id: string, label: string, action: ZOrderAction) =>
    r.register({
      ...
      enabled: hasUnlockedSelection,
      perform: (e) => {
        const map = computeZOrder(
          e.scene.getElements(),
          new Set(unlocked(e).map((x) => x.id)),
          action,
        );
        e.updateElements(
          [...map.entries()].map(([elId, index]) => [elId, { index }] as const),
          label,
        );
      },
    });
  ...
  r.register({
    ...
    enabled: hasSelection,
    checked: (e) =>
      e.getSelectedElements().length > 0 && e.getSelectedElements().every((el) => el.locked),
    perform: (e) => {
      const els = e.getSelectedElements();
      const lock = !els.every((el) => el.locked);
      e.updateElements(
        els.map((el) => [el.id, { locked: lock }] as const),
        lock ? 'Lock' : 'Unlock',
      );
  ...
  r.register<{ ids?: string[] }>({
    ...
    perform: (e, payload) => {
      const ids =
        payload?.ids ??
        (e.state.contextMenu?.elementId ? [e.state.contextMenu.elementId] : e.state.selectedIds);
      e.updateElements(
        ids.map((id) => [id, { locked: false }] as const),
        'Unlock',
      );
      e.select(ids);
    },
  ...
  r.register({
    ...
    perform: (e) => {
      ...
      e.updateElements(
        locked.map((el) => [el.id, { locked: false }] as const),
        'Unlock all',
      );
      e.select(
        locked.map((el) => el.id),
  ...
  r.register({
    ...
    perform: (e) => {
      e.updateElements(
        unlocked(e).map((el) => [el.id, { hidden: true }] as const),
        'Hide',
      );
      e.clearSelection();
    },
  ...
  r.register<{ ids?: string[] }>({
    ...
    perform: (e, payload) => {
      ...
      e.updateElements(
        ids.map((id) => [id, { hidden: false }] as const),
        'Show',
      );
    },
  });
  r.register({
    ...
    perform: (e) => {
      ...
      e.updateElements(
        hidden.map((el) => [el.id, { hidden: false }] as const),
        'Show all',
      );
      e.select(
        hidden.map((el) => el.id),
  ...
  const distribute = (id: string, label: string, mode: 'horizontal' | 'vertical') =>
    r.register({
      ...
      perform: (e) =>
        e.updateElements(distributeElements(unlocked(e), mode, e.state.editingGroupId), label),
    });
  ...
  r.register<{ frameId: string }>({
    ...
    perform: (e, payload) => {
      ...
      e.updateElements(
        els.map((el) => [el.id, { frameId }] as const),
        'Add to frame',
      );
    },
  });
    ...
    perform: (e) =>
      e.updateElements(
        unlocked(e)
          ...
          .map((el) => [el.id, { frameId: null }] as const),
        'Remove from frame',
      ),
  });
  r.register({
  ...
  r.register<{ frameId: string }>({
    ...
    readOnlySafe: true,
    perform: (e, payload) => {
      const frame = payload?.frameId
        ? e.getElement(payload.frameId)
        : e.getSelectedElements().find((el) => el.type === 'frame');
  ...
  r.register({
    ...
    label: 'Reset zoom',
    readOnlySafe: true,
    perform: (e) => e.resetZoom(),
  });
  r.register({
    id: 'view.fitContent',
    label: 'Zoom to fit all',
    readOnlySafe: true,
    perform: (e) => e.fitToContent(),
  });
  r.register({
    id: 'view.fitSelection',
    label: 'Zoom to selection',
    readOnlySafe: true,
    perform: (e) => e.fitToSelection(),
  });
  r.register({
    id: 'view.toggleGrid',
    label: 'Show grid',
  ...
  r.register({
    ...
    perform: (e) =>
      e.setState({ snapping: { ...e.state.snapping, toObjects: !e.state.snapping.toObjects } }),
  });
  r.register({
    ...
    perform: (e) =>
      e.setState({ snapping: { ...e.state.snapping, toGrid: !e.state.snapping.toGrid } }),
  });
  r.register({
    id: 'view.toggleTheme',
    label: 'Dark mode',
  ...
  r.register({
    ...
    label: 'Present',
    readOnlySafe: true,
    perform: (e) => e.startPresentation(),
  });
  r.register({
    id: 'view.shortcuts',
    label: 'Keyboard shortcuts',
    readOnlySafe: true,
    perform: (e) => e.requestUi({ type: 'shortcuts' }),
  });
  r.register({
    id: 'view.commandPalette',
    label: 'Command palette',
    readOnlySafe: true,
    perform: (e) => e.requestUi({ type: 'command-palette' }),
  });
  r.register({
        ...
        const patches = els.flatMap((el): (readonly [string, ElementPatch])[] => {
          if (el.type === 'text') {
            const fontSize = Math.max(
              4,
              Math.min(400, Math.round(el.fontSize * (dir > 0 ? 1.15 : 1 / 1.15))),
            );
            const size = measureTextElement({ ...el, fontSize });
            return [
              [
                el.id,
                { fontSize, width: el.autoResize ? size.width : el.width, height: size.height },
              ],
            ];
          }
              ...
              [
                el.id,
                {
                  label: {
                    ...
                    fontSize: Math.max(
                      4,
                      Math.round(el.label.fontSize * (dir > 0 ? 1.15 : 1 / 1.15)),
                    ),
                  },
                },
              ],
  ...
  const nudge = (id: string, label: string, dx: number, dy: number) =>
    r.register<{ large?: boolean }>({
      ...
      enabled: hasUnlockedSelection,
      perform: (e, payload) => {
        const step = payload?.large
          ? e.state.snapping.toGrid
            ? e.state.grid.size
            : 10
          : e.state.snapping.toGrid
            ? e.state.grid.size
        ...
        for (const el of els)
          if (el.type === 'frame')
            for (const c of e.scene.getFrameChildren(el.id))
              if (!c.locked) withChildren.set(c.id, c);
        e.mutate(
          'Nudge',
          (tx) => {
            tx.updateMany(
              [...withChildren.values()].map(
                (el) => [el.id, { x: el.x + dx * step, y: el.y + dy * step }] as const,
              ),
            );
          ...
          {
            merge:
              e.history.peekUndo()?.label === 'Nudge' &&
              Date.now() - (e.history.peekUndo()?.timestamp ?? 0) < 1000,
          },
        );
      },
    });
  ...
  r.register<{ kind?: AutoLayoutKind; direction?: 'TB' | 'LR' | 'BT' | 'RL' }>({
    ...
    perform: (e, payload) => {
      ...
      const nodes = selected.filter(
        (el) => !isLinearElement(el) && el.type !== 'freedraw' && !el.locked,
      );
      ...
      const edges = e
        ...
        .filter(
          (l) =>
            l.startBinding &&
            l.endBinding &&
            nodeIds.has(l.startBinding.elementId) &&
            nodeIds.has(l.endBinding.elementId),
        );
  ...
  r.register({
    ...
    enabled: hasSelection,
    perform: (e) =>
      e.setState({ selectedIds: selectConnectedComponent(e.scene, e.state.selectedIds) }),
  });
  r.register<{ direction?: 'right' | 'down' | 'left' | 'up' }>({
    id: 'diagram.addConnected',
    label: 'Add connected node',
    enabled: (e) =>
      e.getSelectedElements().length === 1 && !isLinearElement(e.getSelectedElements()[0]!),
  ...
  r.register({
    ...
    label: 'Shape library',
    readOnlySafe: false,
    perform: (e) => e.requestUi({ type: 'library' }),
  });
  ...
  r.register({
    ...
    label: 'Export board',
    readOnlySafe: true,
    perform: (e) => e.requestUi({ type: 'export', scope: 'board' }),
  });
  r.register({
    id: 'export.selection',
  ...
  r.register<{ frameId?: string }>({
    ...
    perform: (e, payload) =>
      e.requestUi({
        type: 'export',
        scope: 'frame',
        frameId: payload?.frameId ?? e.getSelectedElements().find((el) => el.type === 'frame')?.id,
      }),
  });
}
...
function addConnectedNode(e: Editor, direction: 'right' | 'down' | 'left' | 'up') {
  ...
  const offset =
    existing * (direction === 'right' || direction === 'left' ? height + 24 : width + 24) * 0.5;
  const pos = {
  ...
  e.mutate('Add connected node', (tx) => {
    ...
    const route = computeConnectorRoute(
      createdConnector as typeof connector,
      (id) => e.scene.getLiveElement(id),
      [],
    );
