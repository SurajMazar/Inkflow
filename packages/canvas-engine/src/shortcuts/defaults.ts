export type ShortcutCategory =
  'tools' | 'edit' | 'view' | 'arrange' | 'text' | 'navigation' | 'diagram';
...
export const DEFAULT_SHORTCUTS: readonly ShortcutDefinition[] = [
  ...
  },
  {
    ...
    label: 'Select connected diagram',
    category: 'diagram',
    keys: ['Alt+Mod+A'],
  },
  {
...
export function formatShortcut(combo: string, platform: 'mac' | 'other'): string {
  const parts = combo
    .split('+')
    .filter((p, i, arr) => p !== '' || (i === arr.length - 1 && combo.endsWith('++')));
  const keys = combo.endsWith('++')
    ? [...combo.slice(0, -2).split('+').filter(Boolean), '+']
    : parts;
