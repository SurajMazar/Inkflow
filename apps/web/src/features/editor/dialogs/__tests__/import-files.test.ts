...
function setup(options: { readOnly?: boolean } = {}) {
  ...
  const uploadImage = vi.fn(
    async (file: File, fileId: string): Promise<FileMetadata> => ({
      id: fileId,
      mimeType: file.type,
...
const nativeFile = (name = 'board.inkflow') => {
  ...
  const text = createElement('text', { id: 'orig-text', x: 20, y: 10, width: 40, height: 20, text: 'Hi' });
  return new File([JSON.stringify(serializeDocument([rect, text], DEFAULT_DOCUMENT_APP_STATE, {}))], name, { type: 'application/json' });
};
...
describe('importFilesIntoEditor', () => {
  ...
  it('converts Excalidraw files and re-uploads their embedded images', async () => {
    ...
    const excalidraw = {
      ...
      version: 2,
      elements: [
        { id: 'r1', type: 'rectangle', x: 0, y: 0, width: 100, height: 60, angle: 0, strokeColor: '#1e1e1e', backgroundColor: 'transparent', groupIds: [], seed: 1, version: 1, versionNonce: 1, isDeleted: false },
        { id: 'img1', type: 'image', x: 200, y: 0, width: 160, height: 100, angle: 0, groupIds: [], seed: 2, version: 1, versionNonce: 1, isDeleted: false, fileId: 'file1', status: 'saved', scale: [1, 1] },
      ],
      files: { file1: { id: 'file1', mimeType: 'image/png', dataURL: PNG_DATA_URL, created: 1 } },
    };
    await importFilesIntoEditor(editor, [new File([JSON.stringify(excalidraw)], 'sketch.excalidraw')]);
  ...
  it('inserts plain images through the editor image pipeline', async () => {
    ...
    const jpg = new File([new Uint8Array([0xff, 0xd8, 0xff, 0xe0, 0, 0x10])], 'photo.jpg', { type: 'image/jpeg' });
  ...
  it('restores the scene embedded in an exported PNG', async () => {
    const { editor, insertImages } = setup();
    const doc = serializeDocument([createElement('ellipse', { id: 'e1', x: 0, y: 0, width: 80, height: 80 })], DEFAULT_DOCUMENT_APP_STATE, {});
    ...
    const bytes = new Uint8Array([...PNG_BYTES.subarray(0, PNG_BYTES.length - 12), ...scene, ...PNG_BYTES.subarray(PNG_BYTES.length - 12)]);
    await importFilesIntoEditor(editor, [new File([bytes as BlobPart], 'export.png', { type: 'image/png' })]);
    expect(insertImages).not.toHaveBeenCalled();
  ...
  it('converts SVG markup into elements, and falls back to an image when nothing converts', async () => {
    const { editor, insertImages } = setup();
    const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="200" height="100"><rect x="10" y="10" width="80" height="40" fill="#ff0000"/><script>alert(1)</script></svg>';
    ...
    const empty = new File(['<svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"></svg>'], 'empty.svg', { type: 'image/svg+xml' });
  ...
  it('builds diagrams from Mermaid files', async () => {
    const { editor } = setup();
    await importFilesIntoEditor(editor, [new File(['flowchart LR\n  A[Start] --> B[End]'], 'flow.mmd', { type: 'text/plain' })]);
  ...
  it('reports unsupported files without inserting anything', async () => {
    const { editor, insertImages } = setup();
    await importFilesIntoEditor(editor, [new File([new Uint8Array([1, 2, 3, 4])], 'archive.zip', { type: 'application/zip' })]);
  ...
  it('refuses to import into a read-only board', async () => {
    ...
    await importFilesIntoEditor(editor, [nativeFile(), new File([PNG_BYTES], 'a.png', { type: 'image/png' })]);
    expect(addElements).not.toHaveBeenCalled();
    expect(insertImages).not.toHaveBeenCalled();
