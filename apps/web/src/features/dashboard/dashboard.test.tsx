...
const retro = makeBoard({ id: 'b2', title: 'Sprint retro', isFavorite: true, updatedAt: '2026-09-19T09:00:00.000Z' });
const shared = makeBoard({ id: 'b3', title: 'Client flow', role: 'VIEWER', updatedAt: '2026-09-18T09:00:00.000Z' });
...
const card = (id: string) => screen.getAllByTestId('board-card').find((el) => el.dataset.boardId === id)!;
...
beforeEach(() => {
  ...
  useDashboardUi.setState({ renameBoard: null, moveBoard: null, shareBoard: null, searchOpen: false, newBoard: { open: false, defaults: {} } });
});
...
describe('dashboard board list', () => {
  it('renders workspace boards (most recent first) with owner, role and favorite state', async () => {
    ...
    expect(within(card('b2')).getByTestId('board-favorite')).toHaveAttribute('aria-pressed', 'true');
  ...
  it('toggles favorites with PUT/DELETE /boards/:id/favorite', async () => {
    ...
    await waitFor(() => expect(within(card('b1')).getByTestId('board-favorite')).toHaveAttribute('aria-pressed', 'true'));
  ...
  it('renames a board from its menu (PATCH /boards/:id)', async () => {
      ...
      baseRoutes([roadmap], [
        {
          method: 'PATCH',
          path: '/boards/b1',
          respond: (call) => ({ body: { ...roadmap, title: (call.body as { title: string }).title } }),
        },
      ]),
    );
    const user = userEvent.setup();
    ...
    await waitFor(() => expect(within(card('b1')).getByText('Product roadmap')).toBeInTheDocument());
  ...
  it('creates a board from the "New board" dialog and opens it', async () => {
    ...
    const mock = installFetchMock(baseRoutes([], [{ method: 'POST', path: '/boards', respond: { status: 201, body: created } }]));
    const user = userEvent.setup();
    renderDashboard();
...
describe('trash view', () => {
  it('restores and permanently deletes boards', async () => {
    const mock = installFetchMock(
      baseRoutes([], [
        { method: 'POST', path: '/boards/b9/restore', respond: { body: { ...trashed, deletedAt: null } } },
    ...
    expect(mock.callsTo('GET', '/boards').some((c) => c.query.get('filter') === 'trash')).toBe(true);
  ...
  it('empties the trash after confirmation', async () => {
    const mock = installFetchMock(
      baseRoutes([], [{ method: 'POST', path: '/boards/trash/empty', respond: { body: { deleted: 1 } } }]),
    );
    const user = userEvent.setup();
    renderDashboard('/w/w1/trash');
    await findCard('b9');
