...
const LAYOUTS: {
...
}[] = [
  {
    kind: 'hierarchical',
    label: 'Hierarchical',
    description: 'Layered flow, minimal crossings',
    directional: true,
  },
  ...
  { kind: 'vertical', label: 'Vertical', description: 'A single column', directional: false },
  {
    ...
    description: 'Organic, clusters stay close',
    directional: false,
  },
];
...
export function AutoLayoutDialog({ onClose }: { onClose(): void }) {
  ...
  const nodes = selected.filter(
    (e) => !isLinearElement(e) && e.type !== 'freedraw' && !e.locked,
  ).length;
  const edges = selected.filter(isLinearElement).length;
  const meta = LAYOUTS.find((l) => l.kind === kind)!;

  const apply = () => {
    const ok = editor.actions.run(
      'diagram.autoLayout',
      meta.directional ? { kind, direction } : { kind },
    );
    if (ok) onClose();
  };
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-lg" data-inkflow-ui data-testid="autolayout-dialog">
        <DialogHeader>
          ...
          <DialogDescription>
            Arranges the selected nodes; connectors between them are re-routed automatically.
            Elements outside the selection are not moved.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="gap-2 sm:justify-between">
          <Button
            ...
            onClick={() => editor.actions.run('diagram.selectConnected')}
            data-testid="autolayout-select-connected"
          >
