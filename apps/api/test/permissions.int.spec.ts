...
const opsBody = (clientId: string) => ({ clientId, batchId: 'b1', ops: [createOp(clientId, element('rectangle'))] });
...
describe('permissions', () => {
  it('hides boards from strangers and enforces viewer/editor/owner roles', async () => {
    ...
    expect((await stranger.post(`/api/boards/${board.id}/operations`, opsBody('s'))).status).toBe(404);
    ...
    const shared = await owner.post<BoardSharingDto>(`/api/boards/${board.id}/shares`, { email: viewer.user!.email, role: 'VIEWER' });
    ...
    const sharedList = await viewer.get<BoardSummaryDto[]>('/api/boards', { query: { filter: 'shared' } });
    expect(sharedList.body.map((b) => b.id)).toEqual([board.id]);
    expect((await viewer.post(`/api/boards/${board.id}/operations`, opsBody('v'))).status).toBe(403);
    ...
    expect((await viewer.post(`/api/boards/${board.id}/operations`, opsBody('v'))).status).toBe(200);
    expect((await viewer.patch<BoardSummaryDto>(`/api/boards/${board.id}`, { title: 'Edited' })).body.title).toBe('Edited');
    expect((await viewer.patch(`/api/boards/${board.id}`, { workspaceAccess: 'NONE' })).status).toBe(403);
    expect((await viewer.delete(`/api/boards/${board.id}`)).status).toBe(403);
    expect((await viewer.patch(`/api/boards/${board.id}/members/${viewerId}`, { role: 'OWNER' })).status).toBe(403);
    expect((await viewer.post(`/api/boards/${board.id}/shares`, { email: uniqueEmail('x'), role: 'OWNER' })).status).toBe(400);

    // Members can leave.
    const left = await viewer.delete<BoardSharingDto>(`/api/boards/${board.id}/members/${viewerId}`);
  ...
  it('derives access from workspace membership and workspaceAccess', async () => {
    ...
    expect((await member.get<BoardDetailDto>(`/api/boards/${board.id}`)).body.board.role).toBe('EDITOR');
    await owner.patch(`/api/boards/${board.id}`, { workspaceAccess: 'VIEWER' });
    expect((await member.get<BoardDetailDto>(`/api/boards/${board.id}`)).body.board.role).toBe('VIEWER');
    ...
    expect((await member.get<BoardDetailDto>(`/api/boards/${board.id}`)).body.board.role).toBe('OWNER');
  ...
  it('grants share-link roles to anonymous visitors and honours revocation and expiry', async () => {
    ...
    expect(resolved.body).toMatchObject({ boardId: board.id, boardTitle: 'Linked', role: 'VIEWER', expiresAt: null });
    ...
    expect(detail.body.board.role).toBe('VIEWER');
    expect((await anon.get(`/api/boards/${board.id}`, { query: { st: link.body.token } })).status).toBe(200);
    expect((await anon.get(`/api/boards/${board.id}`)).status).toBe(401);
    expect((await anon.post(`/api/boards/${board.id}/operations`, opsBody('anon'), { headers })).status).toBe(403);
    expect((await anon.get(`/api/boards/${board.id}/comments`, { headers })).status).toBe(200);
    const comment = await anon.post(`/api/boards/${board.id}/comments`, { body: 'hi', anchor: { type: 'point', x: 0, y: 0 } }, { headers });
    ...
    const editLink = await owner.post<ShareLinkDto>(`/api/boards/${board.id}/share-links`, { role: 'EDITOR' });
    const editHeaders = { 'x-share-token': editLink.body.token };
    expect((await anon.post(`/api/boards/${board.id}/operations`, opsBody('anon'), { headers: editHeaders })).status).toBe(200);

    // Revoked links stop working.
    expect((await owner.delete(`/api/boards/${board.id}/share-links/${link.body.id}`)).status).toBe(200);
    ...
    const expiring = await owner.post<ShareLinkDto>(`/api/boards/${board.id}/share-links`, { role: 'VIEWER', expiresInHours: 1 });
    await t.prisma.shareLink.update({ where: { id: expiring.body.id }, data: { expiresAt: new Date(Date.now() - 1000) } });
    expect((await anon.get(`/api/share-links/${expiring.body.token}`)).status).toBe(404);
    expect((await anon.get(`/api/boards/${board.id}`, { headers: { 'x-share-token': expiring.body.token } })).status).toBe(404);
  ...
  it('turns pending email shares into memberships when the address is verified', async () => {
    ...
    const pending = await owner.post<BoardSharingDto>(`/api/boards/${board.id}/shares`, { email, role: 'EDITOR', message: 'Welcome!' });
    ...
    const again = await owner.post<BoardSharingDto>(`/api/boards/${board.id}/shares`, { email: uniqueEmail('later') });
    const revoked = await owner.delete<BoardSharingDto>(`/api/boards/${board.id}/shares/${again.body.pending[0]!.id}`);
