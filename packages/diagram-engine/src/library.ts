...
const NODE_SPECS: NodeSpec[] = [
  ...
  },
  {
    ...
    shape: 'parallelogram',
    label: 'Input',
    keywords: ['data', 'io', 'parallelogram'],
  },
  {
    id: 'flow-document',
    name: 'Document',
  ...
  },
  {
    ...
    shape: 'database',
    label: 'Database',
    keywords: ['db', 'sql', 'postgres', 'mysql'],
  },
  {
    id: 'cache',
    name: 'Cache',
  ...
  },
  {
    ...
    shape: 'queue',
    label: 'Queue',
    keywords: ['kafka', 'rabbitmq', 'sqs', 'broker'],
  },
  {
    id: 'container',
    name: 'Container',
  ...
  },
  {
    ...
    icon: 'worker',
    label: 'Worker',
    keywords: ['job', 'background', 'consumer'],
    color: 'orange',
    size: card,
  },
  {
  ...
  },
  {
    ...
    icon: 'globe',
    label: 'DNS',
    keywords: ['domain', 'route53', 'resolver'],
    color: 'teal',
    size: card,
  },
  {
  ...
  },
  {
    ...
    shape: 'function',
    label: 'Function',
    keywords: ['lambda', 'faas'],
  },
  {
    id: 'bucket',
    name: 'Object storage',
    category: 'cloud',
    shape: 'storage-bucket',
    label: 'Bucket',
    keywords: ['s3', 'blob', 'gcs'],
  },
  {
    id: 'cdn',
    name: 'CDN',
    ...
    shape: 'cdn',
    label: 'CDN',
    keywords: ['cloudfront', 'edge', 'akamai'],
  },
  {
    id: 'event-bus',
    name: 'Event bus',
    ...
    shape: 'queue',
    label: 'Event bus',
    keywords: ['eventbridge', 'pubsub', 'stream'],
    color: 'grape',
  },
  {
    id: 'auth-service',
    ...
    shape: 'lock',
    label: 'Auth',
    keywords: ['iam', 'oauth', 'sso', 'identity'],
  },
  {
    id: 'monitoring',
    name: 'Monitoring',
    ...
    icon: 'chart',
    label: 'Monitoring',
    keywords: ['metrics', 'grafana', 'prometheus'],
    color: 'violet',
    size: card,
  },
  {
  ...
  },
  {
    ...
    icon: 'search',
    label: 'Search',
    keywords: ['elasticsearch', 'index', 'opensearch'],
    color: 'teal',
    size: card,
  },
  {
  ...
  },
  {
    ...
    icon: 'package',
    label: 'Package',
    keywords: ['npm', 'library', 'dependency'],
    color: 'yellow',
    size: card,
  },
  {
  ...
  // Data
  {
    ...
    icon: 'layers',
    label: 'Warehouse',
    keywords: ['bigquery', 'snowflake', 'redshift'],
    color: 'violet',
  },
  {
    id: 'data-lake',
  ...
  },
  {
    ...
    icon: 'gear',
    label: 'ETL',
    keywords: ['transform', 'pipeline', 'batch'],
    color: 'orange',
    size: card,
  },
  {
  ...
  // UML
  {
    ...
    shape: 'actor',
    label: 'Actor',
    keywords: ['stick figure', 'use case'],
  },
  {
    id: 'uml-use-case',
    name: 'Use case',
  ...
  return {
    ...
    create(center: Point) {
      ...
      const colors = spec.color
        ? { backgroundColor: PALETTE[spec.color].bg, strokeColor: PALETTE[spec.color].stroke }
        : {};
...
const COMPOSITES: LibraryItem[] = [
  composite(
    ...
    '3-tier web app',
    'software',
    ['architecture', 'web', 'frontend', 'backend', 'database'],
    (b) => {
      const web = b.node('browser', 0, 0, 'Web client');
      const lb = b.node('load-balancer', 260, 25, 'Load balancer');
      const app1 = b.node('rounded-rectangle', 520, -60, 'App server 1', {
        icon: 'server',
        color: 'blue',
        width: 170,
        height: 90,
      });
      const app2 = b.node('rounded-rectangle', 520, 120, 'App server 2', {
        icon: 'server',
        color: 'blue',
        width: 170,
        height: 90,
  ...
  composite(
    ...
    'Publish / subscribe',
    'software',
    ['events', 'kafka', 'producer', 'consumer', 'broker'],
    (b) => {
      const producer = b.node('rounded-rectangle', 0, 70, 'Producer', {
        icon: 'upload',
        color: 'green',
        width: 160,
        height: 90,
      ...
      const c1 = b.node('rounded-rectangle', 500, 0, 'Consumer A', {
        icon: 'worker',
        color: 'orange',
        width: 160,
        height: 90,
      });
      const c2 = b.node('rounded-rectangle', 500, 150, 'Consumer B', {
        icon: 'worker',
        color: 'orange',
        width: 160,
        height: 90,
  ...
  composite('cache-aside', 'Cache-aside', 'data', ['cache', 'redis', 'read through'], (b) => {
    const app = b.node('rounded-rectangle', 0, 60, 'Service', {
      icon: 'server',
      color: 'blue',
      width: 160,
      height: 90,
  ...
  composite(
    ...
    (b) => {
      ...
      const e = b.node('terminator', 5, 260, 'End', {
        backgroundColor: PALETTE.red.bg,
        strokeColor: PALETTE.red.stroke,
      });
  ...
  composite(
    ...
    'ER one-to-many',
    'er',
    ['relationship', 'foreign key', 'crow foot'],
    (b) => {
      const users = b.add(
        createErTable('users', [
          { name: 'id', dataType: 'uuid', primaryKey: true },
  ...
  composite('uml-interface', 'UML interface', 'uml', ['interface', 'contract', 'abstract'], (b) => {
    b.add(
      createUmlClass('Repository', [], ['+ find(id: string): T', '+ save(entity: T): void'], {
  ...
  composite(
    ...
    'UML inheritance',
    'uml',
    ['extends', 'generalization', 'subclass'],
    (b) => {
      const base = b.add(
        createUmlClass('Animal', ['# name: string'], ['+ speak(): void'], { x: 90, y: 0 }),
      );
  ...
  composite(
    ...
    'Sequence diagram',
    'uml',
    ['sequence', 'interaction', 'messages', 'lifeline'],
    (b) => {
      b.add(
        createSequenceDiagram(
          [
            { name: 'Client', kind: 'actor' },
            { name: 'Server' },
  ...
  composite(
    ...
    'Swimlanes',
    'flowchart',
    ['lanes', 'responsibility', 'cross-functional'],
    (b) => {
  ...
  composite(
    ...
    (b) => {
      const svc = b.node('hexagon', 0, 0, 'Orders service', {
        icon: 'microservice',
        color: 'violet',
        width: 180,
        height: 100,
