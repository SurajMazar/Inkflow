...
function plan(
  ...
  extra: Partial<Parameters<typeof planBatch>[0]> = {},
) {
  return planBatch({
    boardId: 'board-1',
    startSeq: 10,
...
describe('operation batches', () => {
  it('validates structure and the connection client id', () => {
    const parsed = parseOps(
      [
        op({ type: 'DELETE_ELEMENT', elementId: 'a' }),
        { type: 'NOPE' },
        op({ type: 'DELETE_ELEMENT', elementId: 'a' }, 'c2'),
      ],
      'c1',
    );
  ...
  it('assigns consecutive seqs to applied ops and lets later ops see earlier results', () => {
    ...
    expect(result.finalStates.get('a')).toMatchObject({
      x: 5,
      y: 6,
      strokeColor: '#f00',
      version: 4,
  ...
  it('reports duplicates from the log and within the batch', () => {
    ...
    const result = plan([known, repeated, repeated], [], {
      duplicates: new Map([[known.opId, 7]]),
    });
    expect(result.results.map((r) => [r.status, r.seq])).toEqual([
      ['duplicate', 7],
      ['applied', 11],
  ...
  it('tracks live element counts through deletes and revivals', () => {
    ...
    const result = plan(
      [
        op({ type: 'DELETE_ELEMENT', elementId: 'x' }),
        op({ type: 'UPDATE_ELEMENT', elementId: 'y', patch: { isDeleted: false } }),
      ],
      existing,
    );
  ...
  it('rejects images pointing at files of other boards but accepts unknown files', () => {
    ...
    expect(checkResultingElements([img], state, 'board-1', { boardOf: () => 'board-2' })).toContain(
      'another board',
    );
    expect(
      checkResultingElements([img], state, 'board-1', { boardOf: () => 'board-1' }),
    ).toBeNull();
    ...
    state.set('img', img);
    expect(
      checkResultingElements([{ ...img, x: 5 }], state, 'board-1', { boardOf: () => 'board-2' }),
    ).toBeNull();
  ...
  it('rejects elements containing NUL characters', () => {
    const result = plan([
      op({
        type: 'CREATE_ELEMENT',
        element: createElement('text', { id: 't', text: 'a\u0000b', width: 5, height: 5 }),
      }),
    ]);
