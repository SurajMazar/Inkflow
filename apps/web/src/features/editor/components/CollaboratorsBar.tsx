        ...
        <Tooltip key={c.clientId}>
          <TooltipTrigger asChild>
            <button
              ...
              role="listitem"
              aria-label={
                following === c.clientId ? `Stop following ${c.name}` : `Follow ${c.name}`
              }
              onClick={() =>
                editor.followCollaborator(following === c.clientId ? null : c.clientId)
              }
              className={cn(
                'rounded-full ring-2 ring-background transition hover:z-10 hover:scale-105',
                following === c.clientId && 'z-10',
              )}
          ...
          <TooltipContent>
            <span className="flex items-center gap-1">
              ...
              {c.name}
              {c.anonymous ? ' (guest)' : ''} ·{' '}
              {following === c.clientId ? 'following' : 'click to follow'}
            </span>
          </TooltipContent>
        </Tooltip>
      ))}
