...
describe('operations (HTTP fallback)', () => {
  it('persists operations in PostgreSQL with consecutive seqs and idempotent retries', async () => {
    ...
    const ops = [
      createOp(clientId, rect),
      createOp(clientId, text),
      moveOp(clientId, 'r1', 300, 400),
    ];
    const res = await owner.post<BatchResponse>(`/api/boards/${board.id}/operations`, {
      clientId,
      batchId: 'batch-1',
      ops,
    });
    ...
    const rows = await t.prisma.boardElement.findMany({
    ...
    const log = await t.prisma.boardOperation.findMany({
      where: { boardId: board.id },
      orderBy: { seq: 'asc' },
    });
    ...
    expect(detail.body.document.elements.map((e) => (e as { id: string }).id)).toEqual([
      't1',
      'r1',
    ]);

    // Retrying the same ops is a no-op reported as duplicate.
    const retry = await owner.post<BatchResponse>(`/api/boards/${board.id}/operations`, {
      clientId,
      batchId: 'batch-1',
      ops,
    });
  ...
  it('rejects invalid operations without aborting the batch', async () => {
    ...
    const ops = [
      ...
      op(clientId, {
        type: 'UPDATE_ELEMENT',
        elementId: 'e1',
        patch: { strokeColor: '#ff0000', id: 'hijack', version: 999 },
      }),
    ...
    const row = await t.prisma.boardElement.findUniqueOrThrow({
      where: { boardId_elementId: { boardId: board.id, elementId: 'e1' } },
    });
    expect(row.isDeleted).toBe(true);
    expect((await t.prisma.board.findUniqueOrThrow({ where: { id: board.id } })).elementCount).toBe(
      0,
    );
    const detail = await owner.get<BoardDetailDto>(`/api/boards/${board.id}`);
    ...
    expect(
      (await owner.post(`/api/boards/${board.id}/operations`, { clientId, batchId: 'b3', ops: [] }))
        .status,
    ).toBe(400);
  });
  ...
  it('merges concurrent edits per property and serves changes since a seq', async () => {
    ...
    await owner.post(`/api/boards/${board.id}/operations`, {
      ...
      ops: [
        op(
          'a',
          { type: 'UPDATE_ELEMENT', elementId: 'shared', patch: { strokeColor: '#111111' } },
          1,
        ),
      ],
    });
    const second = await owner.post<BatchResponse>(`/api/boards/${board.id}/operations`, {
      ...
      ops: [
        op(
          'b',
          { type: 'UPDATE_ELEMENT', elementId: 'shared', patch: { backgroundColor: '#222222' } },
          1,
        ),
      ],
    });
    expect(second.body.changes[0]!.elements[0]).toMatchObject({
      strokeColor: '#111111',
      backgroundColor: '#222222',
      version: 4,
    });

    const since = await owner.get<{ seq: number; changes: ServerChange[] | null }>(
      `/api/boards/${board.id}/changes`,
      { query: { since: 1 } },
    );
    ...
    const none = await owner.get<{ changes: ServerChange[] }>(`/api/boards/${board.id}/changes`, {
      query: { since: 3 },
    });
    ...
    const gap = await owner.get<{ changes: ServerChange[] | null }>(
      `/api/boards/${board.id}/changes`,
      { query: { since: 0 } },
    );
