...
describe('shape registry', () => {
  ...
  it.each(shapeRegistry.list().map((d) => [d.key, d] as const))(
    '%s geometry is closed and within bounds',
    (key, def) => {
      for (const [w, h] of [
        ...
        const node = createElement('node', {
          ...
          width: w,
          height: h,
          customPath: key === 'custom' ? 'M50 0 L100 100 L0 100 Z' : null,
        });
  ...
  it('falls back to rectangle for unknown shapes and scales custom paths', () => {
    ...
    const custom = getNodeGeometry(
      createElement('node', {
        shape: 'custom',
        width: 200,
        height: 50,
        customPath: 'M0 0 L100 0 L50 100 Z',
      }),
    );
    const b = pathBounds(custom.outline);
    expect(b.maxX).toBeCloseTo(200);
    expect(b.maxY).toBeCloseTo(50);
    const broken = getNodeGeometry(
      createElement('node', { shape: 'custom', width: 80, height: 40, customPath: 'M0 0 L1' }),
    );
  ...
  it('respects roundness on rectangles and mirrors flipped geometry', () => {
    const sharp = getNodeGeometry(
      createElement('node', { shape: 'rectangle', width: 100, height: 60, roundness: 'sharp' }),
    );
    const round = getNodeGeometry(
      createElement('node', { shape: 'rectangle', width: 100, height: 60, roundness: 'round' }),
    );
