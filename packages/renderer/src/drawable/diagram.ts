...
function strokeOnly(sets: DrawOpSet[], stroke: StrokePaint | null, alpha?: number): ShapeLayer {
  const layer: ShapeLayer = {
    kind: 'shape',
    sets: sets.filter((s) => s.type === 'stroke'),
    stroke,
    fill: null,
    sketch: null,
    fillRule: 'nonzero',
...
function lineLayer(
  ...
  const sets = roughPath(
    ...
    roughOptionsFor(el, salt, {
      width: Math.abs(x2 - x1) + 20,
      height: Math.abs(y2 - y1) + 20,
      disableMultiStroke: stroke?.dash != null,
    }),
    false,
  );
...
export function iconLayers(key: string, box: Rect, color: string): DrawLayer[] {
  ...
  if (strokes.length)
    layers.push({
      kind: 'shape',
      sets: strokes,
      stroke: solidPaint(color, Math.max(0.75, 2 * s)),
      fill: null,
      sketch: null,
      fillRule: 'nonzero',
    });
...
export function nodeLayers(el: NodeElement): { layers: DrawLayer[]; labelLayers: DrawLayer[] } {
  ...
  const layers: DrawLayer[] = [
    paintSets(el, roughPath(geom.outline, roughOptionsFor(el, 0), filled)),
  ];
  const stroke = strokePaint({ ...el, strokeStyle: 'solid' });
  (geom.details ?? []).forEach((d, i) => {
    layers.push(
      strokeOnly(
        roughPath(d, roughOptionsFor(el, 20 + i, { disableMultiStroke: false }), false),
        stroke,
      ),
    );
  });
  ...
  if (geom.subtitleBox && subtitle) {
    ...
    const { layer } = textBlock({
      text: subtitle,
      style: {
        fontFamily: el.label?.fontFamily ?? 'sans',
        fontSize,
        fontWeight: 'normal',
        fontStyle: 'normal',
        lineHeight: 1.25,
  ...
  const labelBox = geom.labelBox ?? {
    x: 8,
    y: 8,
    width: Math.max(1, el.width - 16),
    height: Math.max(1, el.height - 16),
  };
...
export function tableLayers(el: TableElement): DrawLayer[] {
  ...
  if (filled)
    layers.push({
      kind: 'shape',
      sets: body.filter((s) => s.type === 'fill'),
      stroke: null,
  ...
  if (!isTransparentColor(el.headerColor)) {
    const header = roughPath(
      topRoundedRectPath(0, 0, w, headerH, radius),
      roughOptionsFor(el, 30, { fillStyle: 'solid' }),
      true,
    );
    content.push({
      kind: 'shape',
      sets: header.filter((s) => s.type === 'fill'),
      stroke: null,
      fill: el.headerColor,
      sketch: null,
      fillRule: 'nonzero',
  ...
  content.push(
    textLine(el.name, w / 2, headerH / 2, {
  ...
  if (stroke && el.columns.length > 0)
    content.push(lineLayer(el, 31, 0, headerH, w, headerH, { ...stroke, dash: null }));
  const badgeFontSize = Math.max(8, Math.round(layout.fontSize * 0.8));
  const badgeFont = getFontString({
    fontFamily: el.fontFamily,
  ...
  layout.rows.forEach((row, i) => {
    ...
    content.push(
      textLine(type, layout.typeColumnX, cy, {
        fontFamily: el.fontFamily,
        fontSize: layout.fontSize,
        color: el.strokeColor,
        alpha: 0.65,
...
export function umlClassLayers(el: UmlClassElement): DrawLayer[] {
  ...
  for (const line of layout.nameLines) {
    content.push(
      textLine(line.text, w / 2, line.y + lh / 2, {
        ...text,
        align: 'center',
        bold: line.bold,
        italic: line.italic,
  ...
  if (stroke) {
    ...
    if (layout.attributesY < h)
      content.push(lineLayer(el, 50, 0, layout.attributesY, w, layout.attributesY, sep));
    if (layout.methodsY < h)
      content.push(lineLayer(el, 51, 0, layout.methodsY, w, layout.methodsY, sep));
  }
  for (const line of layout.attributeLines) {
    content.push(
      textLine(line.text, layout.paddingX, line.y + lh / 2, {
        ...text,
        decoration: line.underline ? 'underline' : 'none',
      }),
    );
  }
...
function participantLayers(
  ...
  const opts = (k: number, size: number) =>
    roughOptionsFor(el, salt + k, {
      width: size,
      height: size,
  ...
  switch (p.kind) {
    case 'participant': {
      const path =
        el.roundness === 'round'
          ? roundedRectPath(p.headerX, top, p.headerWidth, p.headerHeight, 4)
          : rectPath(p.headerX, top, p.headerWidth, p.headerHeight);
...
export function sequenceLayers(el: SequenceElement): DrawLayer[] {
  ...
  layout.participants.forEach((p, i) => {
    ...
    if (p.destroyed) {
      ...
      const s = 8;
      layers.push(
        lineLayer(el, 400 + i * 2, p.centerX - s, y - s, p.centerX + s, y + s, {
          ...stroke,
          width: sw * 1.5,
        }),
      );
      layers.push(
        lineLayer(el, 401 + i * 2, p.centerX + s, y - s, p.centerX - s, y + s, {
          ...stroke,
          width: sw * 1.5,
        }),
      );
  ...
  layout.messages.forEach((m, i) => {
    ...
    const opts = roughOptionsFor(el, 600 + i * 3, {
      width: 40,
      height: 40,
    ...
    const hopts = roughOptionsFor(el, 601 + i * 3, {
      width: headLen,
      height: headLen,
      disableMultiStroke: false,
      fillStyle: 'solid',
    ...
    if (m.label) {
      layers.push(
        textLine(m.label, m.labelX, m.labelY, {
          fontFamily: el.fontFamily,
          fontSize: layout.fontSize,
          color: el.strokeColor,
          align: 'center',
  ...
  layout.notes.forEach((n, i) => {
    ...
    const nopts = roughOptionsFor(el, 800 + i * 2, {
      width: n.width,
      height: n.height,
      fillStyle: 'solid',
      disableMultiStroke: false,
    ...
    layers.push({
      kind: 'shape',
      sets: roughPath(outline, nopts, true),
      stroke,
      fill: '#fff9db',
      sketch: null,
      fillRule: 'nonzero',
    ...
    layers.push(
      strokeOnly(
        roughPath(fold, roughOptionsFor(el, 801 + i * 2, { width: f, height: f }), false),
        stroke,
      ),
    );
    ...
    n.lines.forEach((line, li) => {
      layers.push(
        textLine(line, n.x + 8, n.y + 8 + (li + 0.5) * lh, {
          fontFamily: el.fontFamily,
          fontSize: el.fontSize,
          color: el.strokeColor,
        }),
