...
export function buildMicroservices(): TemplateContent {
  ...
  const auth = b.node('rounded-rectangle', 305, 20, 'Auth service', {
    ...card,
    icon: 'lock',
    color: 'yellow',
  });
  const users = b.node('hexagon', 600, 0, 'Users service', {
    ...card,
    icon: 'user',
    color: 'violet',
  });
  const orders = b.node('hexagon', 600, 160, 'Orders service', {
    ...card,
    icon: 'cart',
    color: 'violet',
  });
  const payments = b.node('hexagon', 600, 320, 'Payments service', {
    ...card,
    icon: 'credit-card',
    color: 'violet',
  });
  ...
  const notify = b.node('hexagon', 880, 510, 'Notifications', {
    ...card,
    icon: 'bell',
    color: 'orange',
  });
  const email = b.node('rounded-rectangle', 1130, 510, 'Email provider', {
    ...card,
    icon: 'mail',
    color: 'gray',
  });
  ...
  b.frame(
    'Backend (Kubernetes)',
    [gateway, auth, users, orders, payments, usersDb, ordersDb, paymentsDb, bus, notify],
    40,
  );
  return b.build();
}
...
export function buildRestApi(): TemplateContent {
  ...
  const rate = b.node('rounded-rectangle', 500, 0, 'Rate limiter', {
    ...card,
    icon: 'filter',
    color: 'gray',
  });
  const auth = b.node('rounded-rectangle', 500, 160, 'Auth middleware', {
    ...card,
    icon: 'key',
    color: 'yellow',
  });
  const validate = b.node('rounded-rectangle', 500, 320, 'Validation', {
    ...card,
    icon: 'check',
    color: 'green',
  });
  const r1 = b.node('rounded-rectangle', 760, 0, 'GET /users/:id', {
    ...card,
    color: 'blue',
    width: 190,
  });
  const r2 = b.node('rounded-rectangle', 760, 160, 'POST /orders', {
    ...card,
    color: 'blue',
    width: 190,
  });
  const r3 = b.node('rounded-rectangle', 760, 320, 'DELETE /orders/:id', {
    ...card,
    color: 'blue',
    width: 190,
  });
  ...
  b.text(500, 460, '401 / 403 / 422 errors short-circuit the pipeline', {
    fontSize: 14,
    color: '#868e96',
  });
...
export function buildOAuthSequence(): TemplateContent {
  const b = new DiagramBuilder();
  b.text(0, -70, 'OAuth 2.0 authorization code flow (with PKCE)', {
    fontSize: 28,
    fontWeight: 'bold',
  });
  ...
  seq = sequenceOps.addNote(
    seq,
    [seq.participants[1]!.id, seq.participants[2]!.id],
    'Tokens never pass through the browser URL',
    6,
  );
  b.add(seq);
  b.node(
  ...
  b.node(
    ...
    seq.width + 60,
    170,
    'PKCE: the client sends a hash of a random code_verifier with /authorize and proves possession at /token.',
...
export function buildCiCd(): TemplateContent {
  ...
  const repo = b.node('rounded-rectangle', 180, 10, 'Git repository', {
    ...card,
    icon: 'git-branch',
    color: 'orange',
  });
  const build = b.node('rounded-rectangle', 430, 10, 'Build', {
    ...card,
    icon: 'package',
    color: 'blue',
  });
  const test = b.node('rounded-rectangle', 650, 10, 'Unit tests', {
    ...card,
    icon: 'check',
    color: 'green',
  });
  const scan = b.node('rounded-rectangle', 870, 10, 'Lint & security scan', {
    ...card,
    icon: 'shield',
    color: 'red',
    width: 190,
  });
  const image = b.node('rounded-rectangle', 1110, 10, 'Build image', {
    ...card,
    icon: 'container',
    color: 'cyan',
  });
  const registry = b.node('database', 1120, 230, 'Container registry', { width: 150 });
  const staging = b.node('rounded-rectangle', 870, 240, 'Deploy to staging', {
    ...card,
    icon: 'upload',
    color: 'violet',
    width: 190,
  });
  const e2e = b.node('rounded-rectangle', 650, 240, 'Integration tests', {
    ...card,
    icon: 'terminal',
    color: 'green',
  });
  const approve = b.node('decision', 420, 235, 'Approved?');
  const prod = b.node('rounded-rectangle', 180, 240, 'Deploy to production', {
    ...card,
    icon: 'cloud',
    color: 'violet',
    width: 190,
  });
  const monitor = b.node('rounded-rectangle', 180, 440, 'Monitoring & alerts', {
    ...card,
    icon: 'chart',
    color: 'grape',
    width: 190,
...
export function buildCloudArchitecture(): TemplateContent {
  ...
  const dns = b.node('rounded-rectangle', 150, 470, 'DNS', {
    ...card,
    icon: 'globe',
    color: 'teal',
  });
  const pub = b.node('container', 440, 30, 'Public subnet', {
    width: 240,
    height: 580,
    color: 'green',
  });
  ...
  const app = b.node('container', 720, 30, 'Private subnet — app', {
    width: 320,
    height: 580,
    color: 'blue',
  });
  const asg = b.node('container', 750, 90, 'Auto Scaling group', {
    width: 260,
    height: 490,
    color: 'gray',
    strokeStyle: 'dashed',
  ...
  const servers = [0, 1, 2].map((i) =>
    b.node('rounded-rectangle', 795, 150 + i * 140, `App server ${i + 1}`, {
      ...card,
      icon: 'server',
      color: 'blue',
    }),
  );
  const data = b.node('container', 1080, 30, 'Private subnet — data', {
    width: 340,
    height: 580,
    color: 'violet',
  });
  ...
  b.frame(
    'VPC 10.0.0.0/16',
    [pub, lb, nat, app, asg, ...servers, data, primary, replica, cache],
    30,
  );
  return b.build();
}
...
export function buildKubernetes(): TemplateContent {
  ...
  const webSvc = b.node('rounded-rectangle', 250, 120, 'Service: web', {
    ...card,
    icon: 'network',
    color: 'teal',
  });
  const apiSvc = b.node('rounded-rectangle', 250, 400, 'Service: api', {
    ...card,
    icon: 'network',
    color: 'teal',
  });
  const node1 = b.node('container', 520, 0, 'Worker node 1', {
    width: 260,
    height: 300,
    color: 'gray',
  });
  const node2 = b.node('container', 520, 340, 'Worker node 2', {
    width: 260,
    height: 300,
    color: 'gray',
  });
  const pod = (x: number, y: number, name: string, color: 'blue' | 'violet') =>
    b.node('rounded-rectangle', x, y, name, { ...card, width: 190, icon: 'kubernetes', color });
  ...
  const cfg = b.node('rounded-rectangle', 860, 470, 'ConfigMap & Secrets', {
    ...card,
    icon: 'key',
    color: 'yellow',
    width: 190,
  });
  const hpa = b.node('rounded-rectangle', 860, 40, 'HorizontalPodAutoscaler', {
    ...card,
    icon: 'chart',
    color: 'grape',
    width: 210,
  ...
  b.frame(
    'Cluster: production',
    [ingress, webSvc, apiSvc, node1, node2, web1, api1, web2, api2, pg, cfg, hpa],
    40,
  );
  return b.build();
}
...
export function buildEventDriven(): TemplateContent {
  ...
  const consumers = [
    b.node('rounded-rectangle', 700, -40, 'Email notifier', {
      ...card,
      icon: 'mail',
      color: 'orange',
    }),
    b.node('rounded-rectangle', 700, 100, 'Shipping service', {
      ...card,
      icon: 'map-pin',
      color: 'orange',
    }),
    b.node('rounded-rectangle', 700, 240, 'Analytics', {
      ...card,
      icon: 'pie-chart',
      color: 'orange',
    }),
    b.node('rounded-rectangle', 700, 380, 'Search indexer', {
      ...card,
      icon: 'search',
      color: 'orange',
    }),
...
export function buildFrontendBackend(): TemplateContent {
  ...
  const store = b.node('rounded-rectangle', 10, 190, 'State store', {
    ...card,
    icon: 'layers',
    color: 'blue',
  });
  const client = b.node('rounded-rectangle', 10, 340, 'API client', {
    ...card,
    icon: 'code',
    color: 'blue',
  });
  const cdn = b.node('cdn', 330, -120, 'CDN (static bundle)', { width: 200 });
  const api = b.node('rounded-rectangle', 420, 340, 'API server', {
    ...card,
    icon: 'api',
    color: 'violet',
  });
  const auth = b.node('rounded-rectangle', 700, 190, 'Auth', {
    ...card,
    icon: 'lock',
    color: 'yellow',
  });
  const logic = b.node('rounded-rectangle', 700, 340, 'Business logic', {
    ...card,
    icon: 'gear',
    color: 'violet',
  });
  const jobs = b.node('rounded-rectangle', 700, 490, 'Background jobs', {
    ...card,
    icon: 'worker',
    color: 'orange',
  });
...
export function buildPaymentSystem(): TemplateContent {
  ...
  const paySvc = b.node('hexagon', 440, 120, 'Payment service', {
    ...card,
    icon: 'credit-card',
    color: 'violet',
    width: 190,
  });
  const fraud = b.node('rounded-rectangle', 445, -80, 'Fraud detection', {
    ...card,
    icon: 'shield',
    color: 'red',
    width: 180,
  });
  const psp = b.node('rounded-rectangle', 750, 120, 'Payment gateway (PSP)', {
    ...card,
    icon: 'globe',
    color: 'green',
    width: 210,
  ...
  const orders = b.node('rounded-rectangle', 1040, 290, 'Order service', {
    ...card,
    icon: 'cart',
    color: 'violet',
  });
  const receipts = b.node('rounded-rectangle', 1040, 430, 'Receipts & email', {
    ...card,
    icon: 'mail',
    color: 'orange',
  });
  const recon = b.node('rounded-rectangle', 750, 540, 'Reconciliation job', {
    ...card,
    icon: 'refresh',
    color: 'gray',
    width: 210,
...
export function buildNetworkDiagram(): TemplateContent {
  ...
  const core = b.node('rounded-rectangle', 660, 200, 'Core switch', {
    ...card,
    icon: 'network',
    color: 'green',
  });
  ...
  const access = b.node('rounded-rectangle', 930, 200, 'Access switch', {
    ...card,
    icon: 'network',
    color: 'green',
  });
  const ws = [0, 1, 2].map((i) =>
    b.node('rounded-rectangle', 1200, 40 + i * 130, `Workstation ${i + 1}`, {
      ...card,
      icon: 'laptop',
      color: 'gray',
    }),
  );
  const printer = b.node('rounded-rectangle', 930, 420, 'Printer', {
    ...card,
    icon: 'printer',
    color: 'gray',
  });
  const nas = b.node('rounded-rectangle', 660, 420, 'File server (NAS)', {
    ...card,
    icon: 'hard-drive',
    color: 'blue',
    width: 180,
  });
  const ap = b.node('rounded-rectangle', 1200, 460, 'Wi-Fi access point', {
    ...card,
    icon: 'wifi',
    color: 'teal',
    width: 180,
